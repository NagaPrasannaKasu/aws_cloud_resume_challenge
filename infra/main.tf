...
# create api gateway
resource "aws_apigateway_api" "visitor_counter_api" {
...
resource "aws_apigateway_stage" "default" {
  api_id = aws_apigateway_api.visitor_counter_api.id
  name        = "default"
  auto_deploy = true
}

# api gateway integration
resource "aws_apigateway_integration" "visitor_counter_api_integration" {
  api_id = aws_apigateway_api.visitor_counter_api.id
...
# api gateway route
resource "aws_apigateway_route" "any" {
  api_id = aws_apigateway_api.visitor_counter_api.id
  route_key = "ANY /VisitorCounter"
  target    = "integrations/${aws_apigateway_integration.visitor_counter_api_integration.id}"
}
...
resource "aws_lambda_permission" "api_gateway" {
  ...
  source_arn = "${aws_apigateway_api.visitor_counter_api.execution_arn}/*/*"
}

output "base_url" {
  value = "${aws_apigateway_stage.default.invoke_url}/VisitorCounter"
}